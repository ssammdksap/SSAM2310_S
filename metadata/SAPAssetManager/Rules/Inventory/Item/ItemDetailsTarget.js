...
export default async function ItemDetailsTarget(context) {
    ...
    const propertyRenameStrategy = {
     ...
     return Promise.resolve((propertyRenameStrategy && propertyRenameStrategy(context, item)) || {})
        .then(objWithExtraProps => Object.assign({}, item, commonFields, objWithExtraProps))
        .then(obj => MaterialPlantProps(context, item, type)
        .then(plantProps => Object.assign({}, obj, plantProps)))
        .then(obj => ValidationLibrary.evalIsEmpty(item.PurchaseRequisitionAddress_Nav) ?
            obj : AddressProps(context, item.PurchaseRequisitionAddress_Nav.AddressNumber).then(address => Object.assign({}, address, obj)));
}
...
async function MaterialDocItemProps(context, item) {
    const propObj = {
        ...
        SpecialStockIndicatorText: item.SpecialStockInd ? (await GetSpecialStockIndicatorText(context, item.SpecialStockInd)) : '',
    };
    ...
    } else if (item.MovementType === MovementTypes.t261) {
        propObj.WorkOrder = item.OrderNumber;
    } else if (item.MovementType === MovementTypes.t281) {
        propObj.NetworkActivity = item.Network + '/';
    }
    return propObj;
}

function GetSpecialStockIndicatorText(clientAPI, specialStockInd) {
