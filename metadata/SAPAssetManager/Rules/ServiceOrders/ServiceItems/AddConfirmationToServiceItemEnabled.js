...
export default function AddConfirmationToServiceItemEnabled(context) {
    ...
    return Promise.all(promises).then(results => {
        ...
        let isConfirmationFinalAndCompleted = confirmations.some(confirmation => {
            if (confirmation) {
                let confirmationNav = confirmation.S4ServiceConfirms_Nav || confirmation.S4ServiceConfirmation_Nav;
                let mobileStatus = confirmationNav.MobileStatus_Nav;
                return confirmationNav.FinalConfirmation === 'X' && mobileStatus && mobileStatus.MobileStatus === COMPLETED;
            }
            return false;
        });
