	...
	return Promise.all(reads).then(results => {
		if (results[1].length > 0) { // Edit
			if (context.getName() === 'PartnerPicker1' && !ValidationLibrary.evalIsEmpty(results[1].getItem(0)))
				return `${results[1].getItem(0).PartnerFunction_Nav.Description}${requiredMark}`;
			else if (context.getName() === 'PartnerPicker2' && !ValidationLibrary.evalIsEmpty(results[1].getItem(1)))
				return `${results[1].getItem(1).PartnerFunction_Nav.Description}${requiredMark}`;
			else
				return '';
		} else if (results[0].length > 0) {	// Create
			if (context.getName() === 'PartnerPicker1' && !ValidationLibrary.evalIsEmpty(results[0].getItem(0)))
				return `${results[0].getItem(0).PartnerFunction_Nav.Description}${requiredMark}`;
			else if (context.getName() === 'PartnerPicker2' && !ValidationLibrary.evalIsEmpty(results[0].getItem(1)))
				return `${results[0].getItem(1).PartnerFunction_Nav.Description}${requiredMark}`;
			else
				return '';
		} else {
			return '';
