...
export default function TimeSheetEntryCreateUpdateOnPageLoad(context) {
    ...
    //default WO if applicable
    let woListPicker = context.getControl('FormCellContainer').getControl('RecOrderLstPkr');
    if (!libVal.evalIsEmpty(context.getPageProxy())) {
        ...
        if (!libVal.evalIsEmpty(actionBinding.OrderId)) {
            woListPicker.setValue(`MyWorkOrderHeaders('${actionBinding.OrderId}')`);
        }
    }
