...
export default async function InspectionCharacteristicsUpdateValidation(context, sectionBinding, section) {
    ...
    if (inspCharLib.isQuantitative(sectionBinding) || inspCharLib.isCalculatedAndQuantitative(sectionBinding)) {
        ...
        if (quantitativeControl.getEditable() === true && !libLocal.isNumber(context, value)) {
            return setInlineError(quantitativeControl, context.localizeText('validation_reading_is_numeric'));
        } else {
...
export async function validateComment(section, sectionBinding) {
    ...
    if (commentControl) {
        ...
        if (!comment && isMandatory) {
            libCom.executeInlineControlError(section, commentControl, section.localizeText('comment_is_mandatory'));
            return false;
        }
        ...
        if (comment.length > MaxCommentLength) {
            libCom.executeInlineControlError(section, commentControl, section.localizeText('maximum_field_length', [MaxCommentLength]));
            return false;
        }
    }
...
export async function ValidateDependentCharsEmpty(context, sectionBinding) {
    if (sectionBinding.RequiredChar === 'X') {
        ...
        if (libCom.isDefined(dependentChars)) {
