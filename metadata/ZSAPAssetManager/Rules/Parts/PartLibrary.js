...
export default class {
    ...
    static partCreateUpdateValidation(pageClientAPI) {
        ...
        //validations.push(libThis.validateQuantityGreaterThanZero(pageClientAPI, dict));//Commented to allow Negative Quantity 
        validations.push(libThis.validatePlantNotBlank(pageClientAPI, dict));
        // validations.push(libThis.validateStorageLocationNotBlank(pageClientAPI, dict));///Commented Storagelocation madetory check As MAIN SL is not available
    ...
    static partIssueCreateUpdateValidation(pageClientAPI) {
        ...
        // promises.push(libThis.validateStorageLocationNotBlank(pageClientAPI, dict)); // Added by krishna for Sloc check false
    ...
    static partReturnValidation(pageClientAPI) {
        ...
        let promiseArray = [];
        // promiseArray.push(libThis.validateStorageLocationNotBlank(pageClientAPI, dict)); // Added by krishna for Sloc Validation false
    ...
    */
                ...
                switch (key) {
                    ...
                    case 'Location':
                        if (!libVal.evalIsEmpty(binding.StorageLocation)) {
                            // NCP Defect - Changed value from blank to binding
                            var locationValue = binding.StorageLocation;
                            //var locationValue = '';
                            return sectionProxy.read(
                                '/SAPAssetManager/Services/AssetManager.service',
                                'MaterialSLocs',
    ...
    static getPlantPlusDescription(proxy, plant) {
        // NCP Defect - Changed value from blank to plant
        var plantValue = plant;
        //var plantValue = '';
        try {
            return proxy.read(
                '/SAPAssetManager/Services/AssetManager.service',
