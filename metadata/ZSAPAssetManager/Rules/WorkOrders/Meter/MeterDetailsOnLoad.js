...
//import DataTableVisibility from './ZPreviousMeterReadingVisible'
...
export default function MeterDetailsOnLoad(context) {
    ...
    // let Tablecontrol = context.evaluateTargetPath('#Control:DataTableSection');

    // let OrderId = context.binding.OrderNum;

    // Tablecontrol.setVisible(DataTableVisibility(context, OrderId));
...
export function enableReading(context, binding) {
    if (binding.Device_Nav && binding.Device_Nav.MeterReadings_Nav) {
        ...
        // Request#271 - Take Readings button is not coming for any of the CSD processes
        // Set to true because all migrated last reading in production is 01. 01 is Periodic, not aperoodic
        aperiodicReadingsWithMROFound = true;
