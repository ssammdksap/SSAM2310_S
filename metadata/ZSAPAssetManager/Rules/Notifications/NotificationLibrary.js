...
export default class {
    ...
    static NotificationTaskActivityCreateUpdateCode(context, type) {
        ...
        if (selection.length > 0) {
            ...
            if (notif && (notif['@odata.type'] === '#sap_mobile.MyNotificationItem' || notif['@odata.type'] === '#sap_mobile.MyNotificationTask' || notif['@odata.type'] === '#sap_mobile.MyNotificationActivity')) {
                ...
                // UAT Defect - Added this condition to cater Add Activity on Work Order Details Page
                return context.read('/SAPAssetManager/Services/AssetManager.service', notif['@odata.readLink'] + '/Notification', [], '')
                    .then(function (data) {
                        notif = data.getItem(0);
                        return libThis.CatalogCodeQuery(context, notif, type).then(function (result) {
    ...
    static NotificationTaskActivityGroupQuery(context, type) {
        ...
        if (binding && binding['@odata.type'] === '#sap_mobile.MyNotificationHeader') {
            ...
            // UAT Defect 8000000476 - Add Activity added on Work Order Details Page
            if (binding && binding['@odata.readLink'] && (binding['@odata.type'] === '#sap_mobile.MyNotificationItem' || binding['@odata.type'] === '#sap_mobile.MyNotificationTask' || binding['@odata.type'] === '#sap_mobile.MyNotificationActivity' || binding['@odata.type'] === '#sap_mobile.MyWorkOrderHeader')) {
    ...
    */
    ...
    static ZNotificationCreateUpdateFunctionalLocationPlant(context) {
        let flocValue = context.evaluateTargetPath('#Page:NotificationAddPage/#Control:FuncLocHierarchyExtensionControl').getValue();
        if (!libVal.evalIsEmpty(flocValue)) {
            return context.read('/SAPAssetManager/Services/AssetManager.service', 'MyFunctionalLocations', [], "$filter=FuncLocIdIntern eq '" + flocValue + "'").then(maintPlant => {
                if (maintPlant.getItem.length > 0) {
                    return Promise.resolve(maintPlant.getItem(0).MaintPlant);
                } else {
                    return '';
                }
            });
    ...
    static ZNotificationCreateUpdateEquipmentPlant(context) {
        let equipValue = context.evaluateTargetPath('#Page:NotificationAddPage/#Control:EquipHierarchyExtensionControl').getValue();
        if (!libVal.evalIsEmpty(equipValue)) {
            return context.read('/SAPAssetManager/Services/AssetManager.service', 'MyEquipments', [], "$filter=EquipId eq '" + equipValue + "'").then(maintPlant => {
                if (maintPlant.getItem.length > 0) {
                    return Promise.resolve(maintPlant.getItem(0).MaintPlant);
                } else {
                    return '';
