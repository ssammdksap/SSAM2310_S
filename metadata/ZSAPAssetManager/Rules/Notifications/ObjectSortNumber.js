...
export default function ObjectSortNumber(clientAPI, navlink, fieldname) {
    ...
    if (clientAPI.binding) { // binding may be undefined due to side bar
        // UAT Defect - Added condition to cater Add Activity from Work Order Details Page
        if (clientAPI.binding['@odata.type'] === '#sap_mobile.MyWorkOrderHeader')
            readLink = clientAPI.binding['@odata.readLink'] + '/Notification';
        else
