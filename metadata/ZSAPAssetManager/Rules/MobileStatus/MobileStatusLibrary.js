...
import MobileStatusCompleted from '../../../SAPAssetManager/Rules/MobileStatus/MobileStatusCompleted'; //../../../SAPAssetManager/Rules/MobileStatus
...
export default class {
    ...
    static isMobileStatusConfirmed(context, SubOperation = '', customBinding) {
        ...
        if (!libEval.evalIsEmpty(binding.Confirmations)) {
    ...
    static checkConfirmationsForFinalState(confirmations) {
        if (confirmations && confirmations.length === 0) return false;

        confirmations.sort((a, b) => {
            let keyA = Number(a.ConfirmationCounter);
            let keyB = Number(b.ConfirmationCounter);
        ...
        let lastConfirmation = confirmations[confirmations.length - 1];
        let isLastConfirmationFinal = lastConfirmation? lastConfirmation.FinalConfirmation === 'X' : false;
    ...
    static getQueryOptionsForCompletedStatusForOperations(context, orderId) {
		...
		/* Start*/
		//commented below code to skip Personal number filter on the Operation list picker for Time confirmation screen
